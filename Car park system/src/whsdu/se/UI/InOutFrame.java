...
public class InOutFrame extends JInternalFrame {
	...
	public InOutFrame() {
		...
		bigsumjlabel.setText("total large lot:");			//设置标签文本
		...
		smallsumjlabel.setText("total small lot:");			//设置标签文本
		...
		bigusejlabel.setText("available large lot:");			//设置标签文本
		...
		smallusejlabel.setText("available small lot:");			//设置标签文本
		...
		int b1 = Dal.count("select count(stationid) from sit_infor where stationtype = 'large lot'");

		String a1 = String.valueOf(b1);
		bigsumjtextfield.setText(a1);

		int b2 = Dal.count("select count(stationid) from sit_infor where stationtype = 'small lot'");
		String a2 = String.valueOf(b2);
		smallsumjtextfield.setText(a2);

		int b3 = Dal.count("select count(distinct stationid) from park where stationtype = 'large lot' and startpark  is not null and endpark is null ");

		int b4 = b1 - b3;
		String a3 = String.valueOf(b4);
		bigusejtextfield.setText(a3);

		int b5 = Dal.count("select count(distinct stationid) from park where stationtype = 'small lot'and startpark is not null and endpark is null ");
		int b6 = b2 - b5;
